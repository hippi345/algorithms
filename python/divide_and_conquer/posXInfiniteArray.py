...
def binary_search(binary_search_arr, l, r, x):
    if r >= l:
        mid = l + (r - l) / 2

        if binary_search_arr[int(mid)] == x:
            return mid

        if binary_search_arr[int(mid)] > x:
            return binary_search(binary_search_arr, l, mid - 1, x)

        return binary_search(binary_search_arr, mid + 1, r, x)
...
if ans == -1:
    ...
    print("Element found at index", int(ans))
