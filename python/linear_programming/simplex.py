...
# checks the furthest right column for negative values ABOVE the last row. If negative values exist, another pivot is
# required.
...
# checks that the bottom row, excluding the final column, for negative values. If negative values exist,
# another pivot is required.
...
def loc_piv_r(table):
    ...
    for i, b in zip(col, table[:-1, -1]):
        ...
        if i ** 2 > 0 and b / i > 0:
            ...
            # placeholder for elements that did not satisfy the above requirements. Otherwise, our index number would
            # be faulty.
            total.append(0)
    element = max(total)
    for t in total:
    ...
    if next_round(table):
        ...
        for i, b in zip(table[:-1, n], table[:-1, -1]):
            if i ** 2 > 0 and b / i > 0:
                ...
                # placeholder for elements that did not satisfy the above requirements. Otherwise, our index number
                # would be faulty.
                total.append(0)
        element = max(total)
        for t in total:
...
# The final row of the tablue in a minimum problem is the opposite of a maximization problem so elements are
# multiplied by (-1)
